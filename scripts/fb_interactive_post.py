...
def make_posts(p: dict) -> list[str]:
    ...
    # ── Day 1：地段大考驗（選擇題）────────────────────
    location_options = [
        ("A. 5分鐘", "B. 10分鐘", "C. 15分鐘", "B"),
        ("A. 3分鐘", "B. 8分鐘", "C. 20分鐘", "A"),
        ("A. 走路到", "B. 騎車到", "C. 開車才到", rng.choice(["A", "B"])),
    ]
    loc_q = rng.choice(location_options)
    d1 = f"""【{title}｜地段大考驗 📍】

{emoji} 聽說這個地段超方便，但你相信嗎？
...
#群義房屋 #雲林房屋 #房地產投資 #{ptype}投資 #雲林買房"""
    ...
    fake_one_idx = rng.randint(0, 3)
    fake_one = facilities[fake_one_idx]
    d4 = f"""【{title}｜生活機能大挑戰 🛒】
